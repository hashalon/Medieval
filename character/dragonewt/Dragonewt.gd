...
func is_class(type): return type == "Dragonewt" or .is_class(type)
...
func set_team(team):
	...
	match team:
		...
		TEAM.gamma: bit = 4
	if bit != -1:
		for ray in _rays: ray.set_collision_mask_bit(bit, false)
...
func _ready():
	...
	for ray in _rays: ray.add_exception(self)
...
func _process(delta):
	if is_controlled():
		...
		# prepare cannon ball
		if Input.is_action_just_pressed('secondary'): _is_charging = true
		...
		# release cannon ball
			...
			if _charge_timer > min_charge_time:
				if _charge_timer > max_charge_time: _cannon_ball(1)
				else:
		...
		elif _is_charging: _charge_timer += delta
		
		# fire small sparks (shotgun like)
		elif Input.is_action_just_pressed('attack'): _fire_sparks()
		
		
		# allow the wizard to hover
		if not is_grounded():
			if Input.is_action_pressed('jump') and _velocity.y < 0: _velocity.y = -hover_fall_speed
